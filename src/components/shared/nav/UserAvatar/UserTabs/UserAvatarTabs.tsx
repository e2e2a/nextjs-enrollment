        ...
        <div className='w-full text-left bg-white space-y-1'>
                  ...
                  <div className='flex items-center justify-start gap-2 p-2'>
                    <div className='flex flex-col space-y-1 leading-none'>
                      <p className={`text-[18px] font-medium capitalize`}>
                        {session?.firstname && session?.lastname ? `${session?.firstname} ${session.lastname}`: `${session?.username}`} 
                      </p>
                    </div>
                  </div>
                </Link>
                <div className=' w-full p-2'>
                  <Link href={'/profile'} className='flex items-center w-full p-1 font-medium hover:bg-slate-300 rounded-md justify-center bg-slate-200'>
                    See study load
                  </Link>
                </div>
              </div>
          ...
          {userAvatarItem.map((item, index) => {
            ...
            return item.type === 'button' ? (
              item.title === 'Logout' ? (
                <Button key={index} onClick={() => handleTabChange('')} className={`hover:bg-slate-300 hover:text-white w-full rounded-md justify-start items-center text-[16px] p-2`}>
              ...
              <Link key={index} href={'/feedback'} className={`hover:bg-slate-300 w-full rounded-md px-2 flex justify-start items-center py-1 font-medium text-[16px]`}>
        ...
        {activeTab === 'Account center' && (
          <div className=' transition ease-in-out animate-fadeIn gap-y-6'>
            <div className='flex items-center justify-start py-4'>
              ...
              <p className='font-bold text-[1.5rem] h-full'>Account center</p>
            </div>
            <div className='w-full flex-col text-left items-start bg-white mb-1 mt-1'>
              ...
              {accountCenterItems.map((item, index) => {
                const Icon = Icons[item.icon as keyof typeof Icons];
                return item.href ? (
                  <Link key={index} href={`${item.href}`} className={`hover:bg-slate-300 w-full rounded-md px-2 flex justify-start items-center py-1 font-medium text-[16px]`}>
                    <div className='flex items-center gap-2'>
                      <div className='rounded-full bg-slate-200 p-1'>
                        <Icon />
                      </div>
        ...
        {activeTab === 'Help & support' && (
          <div className=' transition ease-in-out animate-fadeIn gap-y-6'>
            <div className='flex items-center justify-start py-4'>
              ...
              <p className='font-bold text-[1.5rem] h-full'>Help & support</p>
            </div>
        ...
        {activeTab === 'Personal details' && (
          <div className=' transition ease-in-out animate-fadeIn gap-y-6'>
            <div className='flex items-center justify-start pt-4'>
              ...
              <p className='font-bold text-[1.5rem] h-full'>Personal details</p>
            </div>
