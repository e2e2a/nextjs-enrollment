              ...
              <Tooltip>
                <TooltipTrigger className='' asChild>
                  <Link href={item.disabled ? '#' : item.href} className={cn('flex flex-col items-center text-lg font-medium sm:text-sm w-full')}>
                    <Button className='group border border-white hover:bg-slate-200 hover:bg-opacity-45 flex items-center justify-center'>
                      <Icon className={cn('h-5 w-5 lg:h-8 lg:w-8 ', ' stroke-blue-500 scale-[105%]' )} />
                    </Button>
                    <span className={cn( 'text-foreground border-blue-500 border-b-4  ')}></span>
                  </Link>
                </TooltipTrigger>
