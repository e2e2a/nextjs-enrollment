...
export function ComboboxExempted({ form, name, label, selectItems, placeholder, selectedItems, setSelectedItems }: IProps) {
            ...
            <div className='relative bg-slate-50 rounded-lg'>
              <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger id={name} asChild className='w-full pt-10 pb-4 text-left text-black rounded-lg focus:border-gray-400 ring-0 focus:ring-0 px-4'>
                  <Button variant='outline' role='combobox' aria-expanded={open} className='w-full font-normal justify-between truncate' onClick={() => setOpen(!open)}>
                    {field.value && field.value.length > 0 ? field.value.map((value: string) => selectItems.find((item: any) => item.value === value)?.value).join(', ') : placeholder}
