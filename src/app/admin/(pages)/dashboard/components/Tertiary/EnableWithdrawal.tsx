...
const EnableWithdrawal = ({ enrollmentSetup }: IProps) => {
  const [isUploading, setIsUploading] = useState(false);
  const [isOpen, setIsOpen] = useState<boolean | undefined>(false);
  const mutation = useUpdateEnrollmentSetupMutation();

  const actionFormEnable = (openWithdraw: boolean) => {
    setIsOpen(false);
    setIsUploading(true);

    const data = { openWithdraw: openWithdraw };
        ...
        <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
          <AlertDialogTrigger asChild className=' '>
            <Button type='button' disabled={isUploading} variant='outline' size={'sm'} className='animate-fadeOut duration-75 sm:text-sm text-xs bg-blue-500 text-white'>
              <span className=' text-white text-[15px] font-medium'>{isUploading ? <Image src='/icons/buttonloader.svg' alt='loader' width={26} height={26} className='animate-spin' /> : 'Deactivate Withdraw Enrollment'}</span>
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent className='bg-white text-black'>
            <AlertDialogHeader>
              <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
              <AlertDialogDescription className=''>&nbsp;&nbsp;&nbsp;&nbsp;This action will disable the withdrawal of student enrollment.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
          <AlertDialogTrigger asChild>
            <Button type='button' disabled={isUploading} variant='outline' size={'sm'} className='animate-fadeOut duration-75 sm:text-sm text-xs bg-green-500 text-white'>
              <span className=' text-white text-[15px] font-medium'>{isUploading ? <Image src='/icons/buttonloader.svg' alt='loader' width={26} height={26} className='animate-spin' /> : 'Activate Withdraw Enrollment'}</span>
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent className='bg-white text-black'>
            <AlertDialogHeader>
              <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
              <AlertDialogDescription className=''>&nbsp;&nbsp;&nbsp;&nbsp;This action will enable the withdrawal of student enrollment.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
              <AlertDialogAction type='button' className='hidden'>
...
export default EnableWithdrawal;
