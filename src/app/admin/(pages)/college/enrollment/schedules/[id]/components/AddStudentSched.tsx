                ...
                <CommandGroup className='w-full'>
                  <div className='overflow-x-auto w-full '>
                    <div className=' bg-white border border-gray-300'>
                      {schedules.length > 0 &&
                        schedules.map((s: any, index: any) => {
                          // Extract and process values
                          const section = (s.blockTypeId.section ?? '').toLowerCase();
                          const studentType = (studentBlockType ?? '').toLowerCase();
                          const blockYear = (s.blockTypeId.year ?? '').toLowerCase();
                          const year = (studentYear ?? '').toLowerCase();
                          const blockSemester = (s.blockTypeId.semester ?? '').toLowerCase();
                          const semester = (studentSemester ?? '').toLowerCase();
                          if (section === studentType && blockYear === year && blockSemester === semester) {
                            return (
                              <CommandItem className='border w-full block' key={s._id} value={s.subjectId.name}>
                                <div className='flex w-full'>
                                  ...
                                  <div className='flex flex-col text-xs sm:text-sm'>
                                    ...
                                    <span className=' font-semibold'>
                                      Course Code: <span className='uppercase'>{s.courseId.courseCode}</span>
                                    </span>
                                    <span className=' font-semibold'>
                                    ...
                                    <span className=''>
                                      Block: <span className='uppercase'>Block {s.blockTypeId.section}</span>
                                    </span>
                                  </div>
                                </div>
                              </CommandItem>
