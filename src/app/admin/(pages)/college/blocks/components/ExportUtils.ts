...
export const exportToPDF = async (data: any, fileName: string) => {
  return new Promise<void>(async (resolve, reject) => {
    try {
      ...
      const addHeader = () => {
        doc.setFontSize(14);
        doc.text('Blocks Management', pageWidth / 2, yOffset, { align: 'center' });
        yOffset += 8;
      ...
      formattedData = data.map((item: any) => [item?.section ?? 'N/A', item?.courseId?.name ?? 'N/A', item?.courseId?.courseCode ?? 'N/A']);

      let tableHeaders;
      tableHeaders = [['Block Type', 'Department', 'Department Code']];
