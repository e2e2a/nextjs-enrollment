          ...
          <FormControl>
            <div className='relative bg-slate-50 rounded-lg'>
              <Select onValueChange={field.onChange} value={field.value}>
                <SelectTrigger id={name} className='w-full pt-10 pb-4 text-left text-black rounded-lg focus:border-gray-400 ring-0 focus:ring-0 px-4 capitalize'>
                    ...
                    {selectItems.length > 0 ? (
                      selectItems.map((item: any, index: any) => {
                        return item.name ? (
                          ...
                          <SelectItem value={item._id} key={index} className='capitalize'>
                            {item.section} - {item.year} - {item.semester}
                          </SelectItem>
                        ) : (
