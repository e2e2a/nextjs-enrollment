          ...
          <FormControl>
            <div className='relative bg-slate-50 rounded-lg'>
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                <SelectTrigger id={name} className='w-full pt-10 pb-4 font-normal text-left text-black rounded-lg focus:border-gray-400 ring-0 focus:ring-0 px-4'>
                    ...
                    {selectItems && selectItems.length > 0 ?
                      selectItems.map((item: any, index: any) => {
                        return item.subjectCode ? (
                          <SelectItem value={item.subjectCode} key={index} className='capitalize'>
