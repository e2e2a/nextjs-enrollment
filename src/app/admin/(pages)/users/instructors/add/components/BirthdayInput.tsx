...
export function BirthdayInput({ name, form, label, classNameInput }: IProps) {
              ...
              <Popover>
                <div className={`relative w-full`}>
                  <input
                    ...
                    className={`block rounded-xl px-5 pb-2 pt-7 w-full text-sm bg-slate-50 border border-gray-200 appearance-none focus:outline-none focus:ring-0 focus:border-gray-400 peer pl-4 align-text-bottom
                    `}
                    onChange={(e) => {
                      const inputValue = e.target.value;
                      setStringDate(inputValue);
                      const parsedDate = parse(inputValue, 'MM/dd/yyyy', new Date(Date.now()));
