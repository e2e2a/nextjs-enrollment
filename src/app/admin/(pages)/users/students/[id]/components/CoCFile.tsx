...
const CoCFile = ({ user }: { user: any }) => {
  const [fileUrl, setFileUrl] = useState<string | null>(null);
  const [isOpen, setIsOpen] = useState(false);

  useEffect(() => {
    const fetchFileUrl = async () => {
      if (navigator.onLine && user && user.cocUrl) {
        const filePath = `enrollment/coc/${user?._id}/${user?.cocUrl}`;
            ...
            <DialogContent className='max-w-xl bg-white w-full py-10 '>
              ...
              <div className='overflow-y-auto max-h-[400px] '>
                {fileUrl ? (
                  ...
                  <div className='items-center justify-center text-red'>No CoC File</div>
                )}
              </div>
            </DialogContent>
          </Dialog>
