  ...
  return new Promise<void>(async (resolve, reject) => {
    try {
      ...
      const addHeader = () => {
        ...
        doc.text(`Department: ${data?.courseId?.name || 'N/A'}`, pageWidth - 10, yOffset, { align: 'right' });
        yOffset += 6;

        doc.setFontSize(12);
        doc.text(`Descriptive Title: ${data?.subjectId?.name}`, 10, yOffset);
        ...
        yOffset += 6;
        doc.setFontSize(12);
        doc.text(`Year: ${data?.blockTypeId?.year || 'N/A'} - ${data?.blockTypeId?.semester || 'N/A'}`, 10, yOffset);
        doc.text(`Block Type: ${data?.blockTypeId?.section || 'N/A'}`, pageWidth - 10, yOffset, { align: 'right' });
        yOffset += 6;
