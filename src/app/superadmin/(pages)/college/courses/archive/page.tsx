...
const Page = () => {
  ...
  const { data, error } = useCourseQueryByCategory('College');
  ...
  useEffect(() => {
    ...
    if (data) {
      setIsError(false);
      if (data.courses) {
        const a = data.courses.filter((s: any) => s?.archive === true);
        setCourses(a);
      }
          ...
          {isError ? (
            ...
            <div className=''>
              ...
              <div className='flex items-center py-4 text-black w-full justify-center'>
                <h1 className='sm:text-3xl text-xl font-bold '>Archived Courses Management</h1>
              </div>
