  ...
  return new Promise<void>(async (resolve, reject) => {
    try {
      ...
      const addHeader = () => {
        ...
        doc.text(`Time: ${data?.teacherSchedule?.startTime || 'N/A'} - ${data?.teacherSchedule?.endTime || 'N/A'}`, pageWidth - 10, yOffset, { align: 'right' });
        yOffset += 6;
        doc.setFontSize(12);
        doc.text(`Days: ${data?.teacherSchedule?.days}`, 10, yOffset);
        doc.text(`Room: ${data?.teacherSchedule?.roomId?.roomName || 'N/A'}`, pageWidth - 10, yOffset, { align: 'right' });
        yOffset += 6;
        doc.setFontSize(12);
