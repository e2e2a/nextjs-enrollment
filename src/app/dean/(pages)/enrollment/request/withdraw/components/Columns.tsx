...
export const columns: ColumnDef<any>[] = [
  ...
  },
  {
    ...
    accessorKey: 'Enrollment Step',
    header: 'Enrollment Step',
    cell: ({ cell, row }) => {
      const user = row.original;
      return (
        <div key={cell.id} className=' uppercase'>
  ...
  {
    accessorFn: (row) => row?.withdrawApprovedByDean,
    accessorKey: 'withdrawApprovedByDean',
    header: 'Withdraw Approved By Dean',
    cell: ({ cell, row }) => {
      const user = row.original;
      return (
        <div key={cell.id} className=' '>
          {user?.withdrawApprovedByDean ? <span className='uppercase text-green-400 font-semibold'>Approved</span> : <span className='uppercase text-blue-400 font-semibold'>Pending</span>}
        </div>
      );
    },
  },
  {
    accessorFn: (row) => row?.withdrawApprovedByAdmin,
    accessorKey: 'withdrawApprovedByAdmin',
    header: 'Withdraw Approved By Admin',
    cell: ({ cell, row }) => {
      const user = row.original;
      return (
        <div key={cell.id} className=' '>
          {user?.withdrawApprovedByAdmin ? <span className='uppercase text-green-400 font-semibold'>Approved</span> : <span className='uppercase text-blue-400 font-semibold'>Pending</span>}
        </div>
      );
    },
  },
