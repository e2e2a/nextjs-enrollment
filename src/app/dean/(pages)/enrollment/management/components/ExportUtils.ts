...
export const exportToPDF = async (data: any[], fileName: string, step: number) => {
  return new Promise<void>(async (resolve, reject) => {
    try {
      ...
      const addHeader = async () => {
        ...
        doc.setFontSize(14);
        doc.text(`Enrollment Step ${step} Management`, pageWidth / 2, yOffset, { align: 'center' });
        yOffset += 8;
  ...
  const formatData = (data: any) => {
    return data.map((item: any) => {
      ...
      return {
        ...
        'Student Type': item?.studentType ?? 'N/A',
